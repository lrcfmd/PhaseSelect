...
if __name__ == '__main__':
    ...
    epochs = 100
    dirname = 'SUPERCON/Reproduce_ranking'

    # =============== AE RANKING  ================

    # load phase fields of phase vectors if precalculated
    phases = Phase2Vec('', load_phase_vectors='DB/DATA/combined_mpds_scon_Tc.pkl', atomic_mode='embed')
    X = np.array([i for i in phases.dt['phases_vectors'].values if len(i)])

    print(phases.dt.head)
    test = Phase2Vec("", load_phase_vectors='Unexplored_ternaries.pkl', maxlength=phases.maxlength, atomic_mode='embed')
